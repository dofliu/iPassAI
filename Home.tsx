...
import { LEVELS, OFFICIAL_RESOURCE_URL, QUESTIONS, SUBJECTS, type Level, type Question } from "@/data/questions";
...
import { CLAUDE_CERT_SOURCE_URL, CLAUDE_CERT_SUBJECT } from "@/data/claudeCertQuestions";
...
export default function Home() {
  ...
  const librarySubjects = useMemo(() => libraryLevel === "全部" ? LEVELS.flatMap((item) => SUBJECTS[item]) : SUBJECTS[libraryLevel], [libraryLevel]);
  /** 考科索引：把所有級別的科目攤平成 (科目, 級別) 配對，新增級別時自動納入 */
  const courseCatalog = useMemo(() => LEVELS.flatMap((item) => SUBJECTS[item].map((name) => ({ name, level: item }))), []);
  ...
  const subjectAccuracy = useMemo(() => [...SUBJECTS.中級, ...SUBJECTS.專業認證, ...SUBJECTS.初級].map((name) => {
    ...
    <aside className="task-sidebar">
      ...
      <div className="source-note">
        ...
        {subject === CLAUDE_CERT_SUBJECT
          ? "依 Anthropic 官方五大領域考綱權重配題，技術內容對齊官方文件；本站原創題，非官方考古題。"
          : englishMode === CAMBRIDGE_B2_FIRST
          ? "Reading & Use of English Part 1–7、Listening Part 1–4；Listening 以本站原創語音稿播放。"
          : "所有題目均依官方鑑定考綱精準編撰，支援完整考時倒數與及格診斷。"}
        {" "}
        {subject === CLAUDE_CERT_SUBJECT ? (
          <a href={CLAUDE_CERT_SOURCE_URL} target="_blank" rel="noreferrer">
            Claude 認證官方考試資訊 <ExternalLink size={11} />
          </a>
        ) : (
